...
def call_responses_api(
    ...
    *,
    ...
    Payload notes:
    - No ``temperature``: gpt-5 reasoning models reject a non-default temperature
      on the Responses API. Determinism comes from ``reasoning.effort="none"``
      (also what the AS2Web web-search stage uses).
    - ``prompt_cache_key`` is the sha1 of the whole shared instruction block
      (``descr`` + ``fallback_web_search_prompt``, i.e. everything before the
      per-org line, which ``prompt.py`` now keeps at the very end of the
      template). That ~4.6k-token prefix is byte-identical across every request,
      so automatic prompt caching discounts it after the first call; the key
      changes automatically whenever the prompt text changes, so a prompt edit
    ...
    payload: Dict[str, Any] = {
        ...
        "prompt_cache_key": sha1_text(FULL_PROMPT_TEMPLATE),
        "text": {
            "verbosity": "low",
            "format": {
                "type": "json_schema",
