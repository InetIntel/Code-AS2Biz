...
REASON_BROWSER_ERROR_PAGE    = "browser_error_page"
_BROWSER_ERROR_URL_PREFIXES = (
    "chrome-error://",
    "chrome-search://",
    "about:blank",
    "about:neterror",
...
# TLS verification is strict by default. --allow-invalid-tls flips this to add
# Chromium's --ignore-certificate-errors and set the browser context's
# ignore_https_errors, so pages served with an expired/self-signed/mismatched
# certificate are still crawled. Set from the CLI before the browser launches.
ALLOW_INVALID_TLS = False
DEFAULT_WATCHDOG_POLL_INTERVAL = 60
DEFAULT_WATCHDOG_STUCK_THRESHOLD = 600


def _is_browser_error_url(url: str) -> bool:
    """Return True when navigation ended on a browser-internal error page."""
    normalized = (url or "").strip().lower()
    return not normalized or any(
        normalized.startswith(prefix) for prefix in _BROWSER_ERROR_URL_PREFIXES
    )


def _mark_queue_item_done_once(queue, item_token: dict) -> bool:
    """Acknowledge a queue item exactly once across worker/watchdog cleanup."""
    if item_token.get("done"):
        return False
    item_token["done"] = True
    queue.task_done()
    return True


def _chromium_launch_args(disable_browser_sandbox: bool) -> list[str]:
    args = [
        "--headless=new",
...
class Scraper:
    def __init__(
        ...
        watchdog_poll_interval: int = DEFAULT_WATCHDOG_POLL_INTERVAL,
        watchdog_stuck_threshold: int = DEFAULT_WATCHDOG_STUCK_THRESHOLD,
        ...
        self.watchdog_poll_interval = max(1, int(watchdog_poll_interval))
        self.watchdog_stuck_threshold = max(1, int(watchdog_stuck_threshold))
        self.disable_browser_sandbox = bool(disable_browser_sandbox)
        self._worker_last_activity = {}
        self._worker_tasks = {}
        self._retired_worker_tasks = set()
        self._worker_current_item = {}
        self._worker_browser_obj = {}

        import uuid, time as _t
        self.run_id = f"{_t.strftime('%Y%m%dT%H%M%SZ', _t.gmtime())}-{os.getpid()}-{uuid.uuid4().hex[:8]}"

    async def _record_report(self, row: dict):
        """Append one durable report event to memory and the JSONL event log."""
        event = dict(row)
        event["ts"] = time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())
        event["run_id"] = self.run_id
        self.report.append(dict(row))
        ...
        async def run_worker(worker_id, playwright_instance):
            ...
            async def launch_browser():
                for _launch_attempt in range(3):
                    try:
                        launch_args = _chromium_launch_args(self.disable_browser_sandbox)
                        ...
                        self._worker_browser_obj[worker_id] = b
                        return b
                    except Exception as e:
            ...
            while True:
                ...
                self._worker_last_activity[worker_id] = time.monotonic()
                item_token = {"done": False}
                self._worker_current_item[worker_id] = (u, round_no, item_token)
                ...
                try:
                    ...
                    for attempt in range(MAX_RETRIES):
                        ...
                        try:
                            if browser is None or not browser.is_connected():
                                print(f"[worker-{worker_id}] browser disconnected; relaunching", flush=True)
                            ...
                            print(f"[worker-{worker_id}-relaunch-error] {type(e).__name__}: {e}", flush=True)
                    ...
                    _mark_queue_item_done_once(todo_queue, item_token)
                    current_item = self._worker_current_item.get(worker_id)
                    if current_item is not None and current_item[2] is item_token:
                        self._worker_current_item.pop(worker_id, None)

            await close_browser_safely(browser)
            self._worker_browser_obj.pop(worker_id, None)
            print(f"[worker-{worker_id}] finished", flush=True)

        async def external_watchdog(playwright_instance):
            """Replace workers stuck beyond the per-URL retry envelope."""
            while True:
                await asyncio.sleep(self.watchdog_poll_interval)
                now = time.monotonic()
                for worker_id, last_activity in list(self._worker_last_activity.items()):
                    ...
                    if old_task is None or old_task.done():
                        self._worker_last_activity.pop(worker_id, None)
                        continue
                    age = now - last_activity
                    if age <= self.watchdog_stuck_threshold:
                        continue

                    stuck_item = self._worker_current_item.pop(worker_id, None)
                    old_browser = self._worker_browser_obj.pop(worker_id, None)
                    print(
                        f"[external-watchdog] worker-{worker_id} inactive for {age:.0f}s; "
                        "closing its browser and starting a replacement",
                        flush=True,
                    )

                    if stuck_item is not None:
                        url, round_no, item_token = stuck_item
                        if round_no < max_requeue_rounds:
                            todo_queue.put_nowait((url, round_no + 1))
                            disposition = f"requeued at round {round_no + 1}"
                        else:
                            disposition = "requeue limit reached"
                        await self._record_report({
                            "seed": url,
                            "site": canonical_no_www(url),
                            "status": "failed",
                            "reason": REASON_WATCHDOG_TIMEOUT,
                            "details": (
                                f"external worker watchdog after {age:.0f}s; {disposition}"
                            ),
                            "final_url": None,
                            "attempt_no": None,
                        })
                    ...
                    if old_task is not None and not old_task.done():
                        old_task.cancel()
                        self._retired_worker_tasks.add(old_task)
                        old_task.add_done_callback(self._retired_worker_tasks.discard)
                    ...
                    if old_browser is not None:
                        try:
                            ...
                            print(
                                f"[external-watchdog] worker-{worker_id} browser close: "
                                f"{type(e).__name__}: {e}",
                                flush=True,
                            )

                    replacement = asyncio.create_task(run_worker(worker_id, playwright_instance))
                    self._worker_tasks[worker_id] = replacement
                    self._worker_last_activity[worker_id] = time.monotonic()

        # 3) start concurrent execution
        async with async_playwright() as p:
            for worker_id in range(1, CONCURRENCY + 1):
                self._worker_tasks[worker_id] = asyncio.create_task(run_worker(worker_id, p))
            watchdog_task = asyncio.create_task(external_watchdog(p))

            if self._worker_tasks:
                print(f"[start] Launching {len(self._worker_tasks)} dynamic workers...")
            ...
            all_worker_tasks = set(self._worker_tasks.values()) | self._retired_worker_tasks
            unfinished = [task for task in all_worker_tasks if not task.done()]
    ...
    async def _scrape_one_subpage(self, context, link, site, seed_url):
        ...
        try:
            ...
            if _is_browser_error_url(final_link):
                print(f"[sub-browser-error] {link} -> {final_link!r}")
                return
    ...
    async def _scrape_one(self, browser, seed_url, attempt_no: int = 1):
        ...
        async def _finish(status: str, reason: str | None = None,
            ...
            try:
                await self._record_report(entry)
            except Exception as e:
        ...
        try:
            context = await browser.new_context(
                ...
                ignore_https_errors=ALLOW_INVALID_TLS,
            )
            ...
            if _is_browser_error_url(final_main_url):
                await _finish(
                    status="failed",
                    reason=REASON_BROWSER_ERROR_PAGE,
                    details=f"landing navigation ended on browser-internal URL: {final_main_url!r}",
                    final_url=final_main_url,
                )
                return

            # Store the DOM-rendered HTML, not the raw challenge-page response.
            ...
            # ===== Subpages =====
            # Landing success is durable. Optional subpage discovery must not
            # turn it back into a failed seed.
            try:
                ...
                print(
                    f"[subpage-collect-error] {seed_url_norm}: {type(e).__name__}: {e} "
                    "(landing remains archived as success)",
                    flush=True,
                )
...
async def amain(args):
    ...
    try:
        scraper = Scraper(
            ...
            watchdog_poll_interval=args.watchdog_poll_interval,
            watchdog_stuck_threshold=args.watchdog_stuck_threshold,
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        ...
        type=int,
        default=DEFAULT_WATCHDOG_POLL_INTERVAL,
        help="Seconds between external stuck-worker checks.",
    )
    ap.add_argument(
        "--watchdog-stuck-threshold",
        type=int,
        default=DEFAULT_WATCHDOG_STUCK_THRESHOLD,
        help="Replace a worker after this many seconds without task progress.",
    )
    ap.add_argument(
        "--disable-browser-sandbox",
        action="store_true",
        help="Pass Chromium --no-sandbox. Use only inside an appropriately isolated runtime.",
    )
    ap.add_argument(
        "--allow-invalid-tls",
        action="store_true",
        help="Add Chromium --ignore-certificate-errors and set ignore_https_errors so "
             "pages with an expired/self-signed/mismatched certificate are still "
             "crawled. Default is strict TLS verification.",
    )
    args = ap.parse_args()
    ...
    if ALLOW_INVALID_TLS:
        print("[tls] --allow-invalid-tls: TLS certificate errors will be ignored "
              "while crawling.", flush=True)
