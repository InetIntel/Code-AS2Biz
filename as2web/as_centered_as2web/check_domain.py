...
# TLS verification is strict by default. --allow-invalid-tls flips this to
# accept expired/self-signed/hostname-mismatch certificates, for the
# "server answered but its certificate is invalid" reachability definition.
# Set once in main() before any worker thread starts, then read by _session();
# because it never changes after startup, per-thread sessions cannot cache a
# stale value.
...
# ---------------------------------------------------------------------------
# Best-effort guard against probing internal networks
# ---------------------------------------------------------------------------
# This rejects a URL whose host resolves to a loopback / private / link-local /
# reserved address before the first request. It is defence in depth, NOT a
# security boundary: it does not re-check redirect hops, and the resolved
# address can change between this check and the socket connect (DNS rebinding).
# Run the crawler with untrusted input only where it cannot reach sensitive
# internal services (see README, "Responsible crawling").
...
def streamed_get_probe(url: str, timeout: float, retries: int = 1):
    ...
    if not _host_resolves_public(urlparse(url).hostname or ""):
        return "unreachable", None, None, "blocked_non_public_host"
...
def main():
    ...
    ap.add_argument("--allow-invalid-tls", "--allow_invalid_tls",
                    ...
                    help="Accept expired / self-signed / hostname-mismatch TLS "
                         "certificates when probing. Default is strict verification.")
    args = ap.parse_args()
