"""Standalone tests for web_search/main.py TLS and internal-network guards.

Hermetic: socket.getaddrinfo is stubbed, so no live DNS is used.

Run with: python3 as2web/web_search/test_tls_and_ssrf.py
"""
...
def _fresh_reuse_session():
    m._reuse_thread_local.session = None
    return m._reuse_session()


def test_reachability_tls_strict_by_default():
    m.ALLOW_INVALID_TLS = False
    assert _fresh_reuse_session().verify is True


def test_reachability_tls_ignored_only_with_flag():
    m.ALLOW_INVALID_TLS = True
    try:
        assert _fresh_reuse_session().verify is False
    finally:
        m.ALLOW_INVALID_TLS = False
        m._reuse_thread_local.session = None
...
def test_host_resolves_public_classification():
    cases = {
        ...
        "mixed.test":      (["93.184.216.34", "10.0.0.1"], False),
    }
...
def test_check_url_reachable_blocks_non_public_host():
    with _StubDNS({"blocked.test": ["192.168.5.5"]}):
        assert m.check_url_reachable("http://blocked.test/", timeout=1.0) == (False, None)
...
if __name__ == "__main__":
    ...
    print(f"\n{len(tests)} web_search TLS/SSRF tests passed")
