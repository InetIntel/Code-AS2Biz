...
# TLS verification is strict by default for the candidate/previous-snapshot URL
# reachability checks below. --allow-invalid-tls flips it to accept
# expired/self-signed/hostname-mismatch certificates ("server answered but the
# certificate is invalid" still counts as reachable). This applies ONLY to the
# reachability sessions here; the OpenAI API call in fetch_one() uses a separate
# requests.post() and always verifies TLS. Set once in main() before any worker
# thread starts, so per-thread sessions cannot cache a stale value.
...
def _reuse_session() -> requests.Session:
    ...
    if s is None:
        ...
        s.max_redirects = 10
        s.verify = not ALLOW_INVALID_TLS
        _reuse_thread_local.session = s
    return s


def _host_resolves_public(host: str) -> bool:
    """Best-effort: True only if every address ``host`` resolves to is public
    unicast. Defence in depth, NOT a security boundary — it does not re-check
    redirect hops and the address can change before the socket connect (DNS
    rebinding). Run with untrusted input only where internal networks are
    unreachable (see README, "Responsible crawling")."""
    if not host:
        return False
    try:
...
def check_url_reachable(url: str, timeout: float = 10.0) -> tuple:
    ...
    """
    if not _host_resolves_public(urlparse(url).hostname or ""):
        return False, None
...
def main():
    ...
    ap.add_argument("--prev_combined", default=None,
                    help="Path to previous snapshot's as2web.json for the reuse "
                         "heuristic. Default: auto-detect the most recent "
                         "YYYYMMDD/as2web.json under <base_dir>/../combined/.")
    ...
    ap.add_argument("--allow-invalid-tls", "--allow_invalid_tls",
                    dest="allow_invalid_tls", action="store_true",
                    help="Accept expired / self-signed / hostname-mismatch TLS "
                         "certificates during candidate/previous-snapshot URL "
                         "reachability checks. Default is strict verification. Does not "
                         "affect the OpenAI API request, which always verifies TLS.")
    args = ap.parse_args()

    global ALLOW_INVALID_TLS
    ALLOW_INVALID_TLS = args.allow_invalid_tls
    if ALLOW_INVALID_TLS:
        urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)
        logging.warning("--allow-invalid-tls: TLS certificate errors will be ignored "
                        "during URL reachability checks.")
